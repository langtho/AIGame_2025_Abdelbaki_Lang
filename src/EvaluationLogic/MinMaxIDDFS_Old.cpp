...
bool MinMaxIDDFS_Old::z_initialized = false;
uint64_t MinMaxIDDFS_Old::z_red[16][100];
uint64_t MinMaxIDDFS_Old::z_blue[16][100];
uint64_t MinMaxIDDFS_Old::z_trans[16][100];
uint64_t MinMaxIDDFS_Old::z_turn;

MinMaxIDDFS_Old::MinMaxIDDFS_Old(int time_ms) : time_limit_ms(time_ms), time_out(false) {
    if (!z_initialized) initZobrist();
    tt.reserve(200000);
}

void MinMaxIDDFS_Old::initZobrist() {
    std::mt19937_64 rng(12345);
    std::uniform_int_distribution<uint64_t> dist;
    for (int i = 0; i < 16; ++i) {
        for (int j = 0; j < 100; ++j) {
            z_red[i][j] = dist(rng); z_blue[i][j] = dist(rng); z_trans[i][j] = dist(rng);
        }
    }
    z_turn = dist(rng);
    z_initialized = true;
...
uint64_t MinMaxIDDFS_Old::computeHash(const State& state) {
    uint64_t h = 0;
...
int MinMaxIDDFS_Old::_minmax(const State &state, int depth, int alpha, int beta, bool maximizing_player, bool original_player_is_p1) {
    ...
    if (!time_out) {
        TTEntry entry;
        entry.depth = depth; entry.value = value; entry.best_move = best_move_this_node;
...
std::pair<int, Color> MinMaxIDDFS_Old::find_best_move(const State &state) {
